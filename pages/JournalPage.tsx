...
const JOURNAL_STORAGE_KEY = 'elderlyease_journal_messages';
...
const JournalPage: React.FC = () => {
                 ...
                 setMessages([
                    {
                        id: 'init-1',
                        sender: 'ai',
                        text: "Hello, this is your private diary, Echo. You can share anything on your mind with me by typing or by tapping the microphone to speak. Your thoughts are safe here.",
        ...
        utterance.onerror = (event: SpeechSynthesisErrorEvent) => {
            ...
            switch (event.error) {
                ...
                case 'language-unavailable':
                    errorText = "Sorry, I can't speak this language on your device.";
                    break;
    ...
    const handleSendMessage = useCallback(async (text: string, mood?: Mood, ratingValue?: number) => {
        ...
        try {
            const aiResponseText = await getDiaryResponse(currentHistory, 'en-US');
     ...
     useEffect(() => {
        ...
        recognition.onend = () => {
            if (assistantState === 'listening') { // Only if it wasn't stopped manually
                setAssistantState('idle');
            ...
            <div className="flex-1 flex flex-col bg-white rounded-2xl shadow-lg overflow-hidden">
                ...
                <div className="p-4 bg-white border-t border-gray-200">
                    ...
                    <form onSubmit={handleFormSubmit} className="flex items-center space-x-3">
                        <input type="text" value={userInput} onChange={(e) => setUserInput(e.target.value)} placeholder="Type or use the mic to talk..." className="flex-1 bg-gray-100 border-2 border-transparent focus:border-teal-500 rounded-full px-5 py-3 text-lg focus:outline-none" disabled={assistantState !== 'idle'}/>
                         <button 
                            type="button" 
